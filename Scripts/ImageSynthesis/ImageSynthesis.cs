...
public class ImageSynthesis : MonoBehaviour {
	...
	void Start()
	{
		...
		for (int q = 1; q < capturePasses.Length; q++) {
			capturePasses[q].camera = CreateHiddenCamera(this.name+capturePasses[q].name);
            capturePasses[q].camera.targetTexture = Resources.Load<RenderTexture>(this.name+"_view"+capturePasses[q].name);
            Debug.Log("connect: "+this.name+"_view"+capturePasses[q].name);
        }
	...
	public void Save(string filename, int width = -1, int height = -1, string path = "/Users/andrewyu/Documents/work/depth_estimation/")
	{
		if (width <= 0 || height <= 0)
		{
			width = Screen.width;
		...
		var filenameWithExtension = Path.Combine("train", Path.GetFileNameWithoutExtension(filename)+filenameExtension);

		// var pathWithoutExtension = Path.Combine(path, filenameWithoutExtension);

		// execute as coroutine to wait for the EndOfFrame before starting capture
		StartCoroutine(
			WaitForEndOfFrameAndSave(path, filenameWithExtension, width, height)
        );
	}

	private IEnumerator WaitForEndOfFrameAndSave(string pathname, string filename, int width, int height)
	{
		yield return new WaitForEndOfFrame();
		Save(pathname, filename, width, height);
	}

	private void Save(string pathname, string filename, int width, int height)
	{
		foreach (var pass in capturePasses)
			Save(pass.camera, Path.Combine(pathname + pass.name=="_depth"?"annotations":"images", filename), width, height, pass.supportsAntialiasing, pass.needsRescale);
	}
